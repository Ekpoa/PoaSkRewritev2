...
import poa.packets.FakeEntity1219;
import poa.packets.Metadata1219;
import poa.packets.TeamPacket1219;

import java.util.*;
import java.util.concurrent.ThreadLocalRandom;

public class GuardianBeam1219 {
   ...
   // public static Map<String, GuardianBeam121> dataMap = new HashMap<>();
    ...
    public GuardianBeam1219(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1219.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1219.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1219.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1219 guardianMeta = new Metadata1219(this.guardianID);
            ...
            final Metadata1219 squidMeta = new Metadata1219(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1219.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
