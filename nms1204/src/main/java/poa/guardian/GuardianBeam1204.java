...
import poa.packets.Metadata1204;
...
public class GuardianBeam1204 {

    //public static Map<String, GuardianBeam1204> dataMap = new HashMap<>();
    ...
    public GuardianBeam1204(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1204.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1204.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1204.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1204 guardianMeta = new Metadata1204(this.guardianID);
            ...
            final Metadata1204 squidMeta = new Metadata1204(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1204.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
