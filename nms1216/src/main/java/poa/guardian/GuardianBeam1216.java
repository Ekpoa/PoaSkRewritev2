...
import poa.packets.FakeEntity1216;
import poa.packets.Metadata1216;
import poa.packets.TeamPacket1216;

import java.util.*;
import java.util.concurrent.ThreadLocalRandom;

public class GuardianBeam1216 {
   ...
   // public static Map<String, GuardianBeam121> dataMap = new HashMap<>();
    ...
    public GuardianBeam1216(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1216.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1216.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1216.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1216 guardianMeta = new Metadata1216(this.guardianID);
            ...
            final Metadata1216 squidMeta = new Metadata1216(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1216.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
