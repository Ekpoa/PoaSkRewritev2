...
import poa.packets.FakeEntity121;
import poa.packets.Metadata121;
import poa.packets.TeamPacket121;
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity121.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity121.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket121.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata121 guardianMeta = new Metadata121(this.guardianID);
            ...
            final Metadata121 squidMeta = new Metadata121(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity121.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
