...
import poa.packets.Metadata1202;
...
public class GuardianBeam1202 {

    //public static Map<String, GuardianBeam1202> dataMap = new HashMap<>();
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1202.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1202.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1202.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1202 guardianMeta = new Metadata1202(this.guardianID);
            ...
            final Metadata1202 squidMeta = new Metadata1202(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1202.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
