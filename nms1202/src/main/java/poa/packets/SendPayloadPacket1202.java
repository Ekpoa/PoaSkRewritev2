...
public class SendPayloadPacket1202 {
    ...
    public static void sendGameTestMarker(Player player, Location location, String text, Color color, int durationMs) {
        ...
        SendPacket1202.sendPacket(player, new ClientboundCustomPayloadPacket(marker));

    }

    public static void sendBrandPayload(Player player, String brand){
        SendPacket1202.sendPacket(player, new ClientboundCustomPayloadPacket(new BrandPayload(brand)));
    }
