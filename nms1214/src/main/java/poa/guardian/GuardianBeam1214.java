...
import poa.packets.FakeEntity1214;
import poa.packets.Metadata1214;
import poa.packets.TeamPacket1214;

import java.util.*;
import java.util.concurrent.ThreadLocalRandom;

public class GuardianBeam1214 {
   ...
   // public static Map<String, GuardianBeam121> dataMap = new HashMap<>();
    ...
    public GuardianBeam1214(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1214.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1214.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1214.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1214 guardianMeta = new Metadata1214(this.guardianID);
            ...
            final Metadata1214 squidMeta = new Metadata1214(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1214.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
