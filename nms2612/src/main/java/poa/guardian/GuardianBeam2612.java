...
import poa.packets.FakeEntity2612;
import poa.packets.Metadata2612;
import poa.packets.TeamPacket2612;

import java.util.*;
import java.util.concurrent.ThreadLocalRandom;

public class GuardianBeam2612 {
   ...
   // public static Map<String, GuardianBeam121> dataMap = new HashMap<>();
    ...
    public GuardianBeam2612(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity2612.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity2612.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket2612.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata2612 guardianMeta = new Metadata2612(this.guardianID);
            ...
            final Metadata2612 squidMeta = new Metadata2612(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity2612.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
