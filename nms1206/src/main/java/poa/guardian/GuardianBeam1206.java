...
import poa.packets.Metadata1206;
...
public class GuardianBeam1206 {

   // public static Map<String, GuardianBeam1206> dataMap = new HashMap<>();
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity1206.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity1206.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket1206.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata1206 guardianMeta = new Metadata1206(this.guardianID);
            ...
            final Metadata1206 squidMeta = new Metadata1206(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity1206.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
