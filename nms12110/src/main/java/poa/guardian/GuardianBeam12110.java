...
import poa.packets.FakeEntity12110;
import poa.packets.Metadata12110;
import poa.packets.TeamPacket12110;

import java.util.*;
import java.util.concurrent.ThreadLocalRandom;

public class GuardianBeam12110 {
   ...
   // public static Map<String, GuardianBeam121> dataMap = new HashMap<>();
    ...
    public GuardianBeam12110(List<Player> players, String id, Location startLoc, Location endLoc, String color, Plugin plugin) {
...
//        }
    ...
    public void show(CraftPlayer player) {
        try {
            final Object guardianPacket = FakeEntity12110.fakeEntityPacket(this.guardianID, this.guardianLoc, "guardian", this.guardianUUID);
            final Object squidPacket = FakeEntity12110.fakeEntityPacket(this.squidID, this.batLoc, "bat", this.squidUUID);

            final Object teamPacket = TeamPacket12110.teamPacketForBeam(List.of(this.guardianUUID.toString(), this.squidUUID.toString()), this.color);

            final Metadata12110 guardianMeta = new Metadata12110(this.guardianID);
            ...
            final Metadata12110 squidMeta = new Metadata12110(this.squidID);
    ...
    public void destroy() {
        final Object removePacket = FakeEntity12110.removeFakeEntityPacket(List.of(this.guardianID, this.squidID));
        for (UUID uuid : this.uuids) {
            final Player player = Bukkit.getPlayer(uuid);
            if (player == null)
                continue;
